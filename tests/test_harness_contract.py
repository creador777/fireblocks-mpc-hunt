...
class HarnessContractTests(unittest.TestCase):
    ...
    def test_08_the_corpus_subtree_is_a_function_of_the_surface(self):
        ...
        self.assertIn('DESTINATION="${STAGE}/corpus/${HARNESS}"', body)
        self.assertNotIn('corpus/cmp_ecdsa_online"', body,
                         "queda una ruta de corpus fija a una superficie")

    def test_09_the_incident_name_carries_the_surface(self):
        body = PUBLISH_INGEST.read_text(encoding="utf-8")
        self.assertIn("${HARNESS}-attempt-", body,
                      "dos lanes escribirían el mismo nombre de incidente")
    ...
    def test_13_the_aggregator_never_lands_a_unit_in_the_other_subtree(self):
        ...
        self.assertRegex(
            body,
            r"\^corpus/\(cmp_ecdsa_online\|cmp_ecdsa_online_r4_tn\)/",
            "el agregador no nombra las dos superficies por separado")
        self.assertIn("BASH_REMATCH[2]", body,
                      "con dos grupos, el hash es [2]: usar [1] compararía el "
                      "sha1 contra el nombre de la superficie")
